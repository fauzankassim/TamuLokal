...
const MarketCard = ({ market, height = 250 }) => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setDistance((dist / 1000).toFixed(2)); // convert meters → km
      },
      (err) => {
  ...
  const now = new Date();
  const currentDay = now.getDay() + 1; // JS: Sunday=0, Daily schedule: 1=Monday
  ...
  if (todaySchedules.length > 0) {
    ...
    const schedules = (market.schedules || [])
      .map((s) => {
    ...
    if (schedules.length > 0) {
      ...
      if (futureSchedules.length > 0) {
        const sorted = futureSchedules.sort((a, b) => {
          const aDiff =
            a.type === "once" ? a.onceDate - nowTime : ((a.dayOfWeek + 7 - (now.getDay() + 1)) % 7) * 24 * 60 * 60 * 1000;
          const bDiff =
            b.type === "once" ? b.onceDate - nowTime : ((b.dayOfWeek + 7 - (now.getDay() + 1)) % 7) * 24 * 60 * 60 * 1000;
          return aDiff - bDiff;
        });
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-3 left-3 right-3 text-white flex flex-col gap-2">
        <h2 className="font-semibold text-lg drop-shadow leading-tight line-clamp-1">{market.name}</h2>
